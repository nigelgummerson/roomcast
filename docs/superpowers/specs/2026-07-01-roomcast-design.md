...
readable copy that lives in their browser for a bounded time, then self-destructs. No
server, no login, no paper. The document data never crosses a network.
...
commodity; the novelty is the ephemeral, offline, IG-first broadcast model.

## 3. Two modes, one codebase

**Presenter mode** (laptop → projector):
1. Drop a `.docx` (PDF/plain-text later).
2. Parse to markdown (mammoth.js).
3. Compress (fflate / gzip).
4. Fountain-encode + frame (qrloop / BC-UR).
5. Animate one large, low-density QR on a loop (~5–10 fps).
6. Also display a small static **"open the reader"** QR (links phones to the reader
   page) and a confidentiality banner + selected security profile.
...
4. Reassemble → inflate → markdown → render as clean readable HTML.
5. Store in IndexedDB as `{payload, scannedAt, profile}`.
6. Show the document offline with a live expiry countdown and confidentiality banner.

## 4. Pipeline / data flow

```
.docx → mammoth.js (→ markdown) → fflate (gzip) → qrloop/BC-UR (fountain framing)
      → qrcode (render frames) → [projected animation]
      → phone camera → zxing-wasm (decode frames) → qrloop reassembly
      → fflate inflate → markdown → rendered HTML → IndexedDB {payload, scannedAt, profile}
```

Fountain coding is the crux: the phone needs *enough* frames, not every frame, so
missed/blurred frames self-heal — and a single still photo of one frame reconstructs
nothing.

## 5. Security profiles (generalisation)
...
- **docParser** — file → markdown. In: File; Out: markdown string. Depends on mammoth.
...
Vitest unit tests:
- docParser: `.docx` → markdown extraction.
...
Integration: full encode → decode across a lossy channel.
